...
class AML:
    ...
    def f(self, x, c_0, p_0):
        ...
        for i in allIs:
            for j in allIs:
                ...
                possibly_changing_noise1 = self.noise.noise.detach().clone()
                possibly_changing_noise2 = self.noise.noise.detach().clone()
                # Change the noise copy with the randomly chosen vector in direction a
                possibly_changing_noise1[i][j][0] += a * q[0]
                possibly_changing_noise1[i][j][1] += a * q[1]
                possibly_changing_noise1[i][j][2] += a * q[2]

                possibly_changing_noise2[i][j][0] -= a * q[0]
                possibly_changing_noise2[i][j][1] -= a * q[1]
                possibly_changing_noise2[i][j][2] -= a * q[2]
                positive = x + self.noise.to_image_tensor('cuda', noise=possibly_changing_noise1)
                negative = x + self.noise.to_image_tensor('cuda', noise=possibly_changing_noise2)
                c11, p11 = self.internal_model.run(positive)
                c12, p12 = self.internal_model.run(negative)
                if p11 > p12:
                    self.noise.noise = possibly_changing_noise1
                else:
                    self.noise.noise = possibly_changing_noise2
        return c_1, p_1
    ...
    def accuracy(self, eps):
        ...
        for path in files:
            ...
            if c0 != c1 and p1 >= 0.85 * p0:  # or p1 <= x * p0
                correct += 1.0
...
def run_personal2(path, model, eps):
    img_obj = PersonalImage(path)
    x = img_obj.x.cpu()
    c_true, p_true, c_adv, p_adv, x = model.apply_changes(x, eps)
    return c_true, p_true, c_adv, p_adv
...
runs = 500
image = "dog.jpg"
for eps in range(1, runs):
    if eps == runs - 1:
        run_personal("images/personal/" + image, model, highest_prob_eps)
    eps = eps / 1000
    c_true, p_true, c_adv, p_adv = run_personal2("images/personal/" + image, model, eps)
    print("eps", eps, "class", labels[str(c_adv)], "{:.3f}".format(p_adv.item() * 100) + "% probability")
    if p_adv.item() > highest_prob and c_adv != c_true:
        highest_prob = p_adv.item()
        highest_prob_eps = eps
